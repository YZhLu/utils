...
export function increment(
	...
	const inc = size * grade * ((1 - decay) ** distance / nSamples);
	return inc;
}

export function getChain(skillId: string, mapStatus: MapStatus[]): { [id: string]: number } {
	//const mapNodes = skillmap.mapNodes;
	//console.log(skillId)
	const chain: { [id: string]: number } = {};
	chain[skillId] = 0;
	const firstNode = mapStatus.find((mStat) => mStat.mapNode.node.data.id === skillId);
			...
			Object.keys(chain).forEach((id) => {
				...
				mapStatus
					...
					?.mapNode.group.forEach((n: string) => {
						//console.log("node? = ", n)
						const _node = mapStatus.find((_mStat) => _mStat.mapNode.node.id === n);
...
export function getMapNodesChain(mapNodeIds: string[], chain: { [id: string]: number }) {
...
export const updStatus = (
	...
	const increments = getIncrements(grade, size, decay, samples, mapStatus, skillId);
